...
from core.handle.gestureHandle import gesture_prompt, parse_gesture_text
    ...
    async def handle(self, conn: "ConnectionHandler", msg_json: Dict[str, Any]) -> None:
        ...
        if msg_json["state"] == "start":
            ...
            if "text" in msg_json:
                ...
                gesture = parse_gesture_text(original_text)
                if gesture is not None:
                    prompt = gesture_prompt(gesture)
                    if prompt is None:
                        conn.logger.bind(tag=TAG).info(f"忽略未知手势: {gesture}")
                        return
                    conn.logger.bind(tag=TAG).info(f"收到设备手势: {gesture}")
