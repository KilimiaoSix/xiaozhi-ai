...
class WebSocketServer:
    def __init__(self, config: dict):
        ...
        # 设备常连后按 device-id 定位活跃连接，供外部工作事件推送使用
    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        # 先登记再进入阻塞的会话循环，否则设备刚连上的窗口期推送会找不到连接。
        # handle_connection 内部还会再赋一次同一个 websocket，此处提前赋值是安全的。
        device_id = websocket.request.headers.get("device-id", None)
        handler.websocket = websocket
        self.device_registry.register(device_id, handler)
        try:
            await handler.handle_connection(websocket)
        except Exception as e:
            self.logger.bind(tag=TAG).error(f"处理连接时出错: {e}")
        finally:
            self.device_registry.unregister(device_id, handler)
            # 强制关闭连接（如果还没有关闭的话）
            try:
                # 安全地检查WebSocket状态并关闭
