...
| GET | `/xiaozhi/feishu/status` | Server 飞书用户凭据与工作台能力状态 |
| GET | `/xiaozhi/feishu/briefing` | 今日日程与当前用户未完成任务（可选 `date=YYYY-MM-DD`） |
...
4. ~~晨报没有主动送达路径~~ **已接通**（`core/morning_brief/announcer.py`）：时间窗内
   ...
   **向飞书方向的主动发送**（晨报只播给机器人和 HTTP 预览，不会往飞书发消息）。
