...
NoAudioCodecSimplex::NoAudioCodecSimplex(int input_sample_rate, int output_sample_rate, gpio_num_t spk_bclk, gpio_num_t spk_ws, gpio_num_t spk_dout, gpio_num_t mic_sck, gpio_num_t mic_ws, gpio_num_t mic_din) {
    ...
    // RX 使用立体声以确保 WS 时钟周期完整，很多 I2S 麦克风（如 INMP441）在单声道模式下工作不稳
...
int NoAudioCodec::Read(int16_t* dest, int samples) {
    ...
    int actual_samples = bytes_read / sizeof(int32_t) / 2;
    int64_t sum_abs_left = 0, sum_abs_right = 0;
    int32_t peak_left = 0, peak_right = 0;

    for (int i = 0; i < actual_samples; i++) {
        // 左声道提取 (使用 12 位移位以增加数字增益，应对弱信号)
        int32_t val_l = bit32_buffer[i * 2] >> 12;
        int16_t sample_l = (val_l > INT16_MAX) ? INT16_MAX : (val_l < -INT16_MAX) ? -INT16_MAX : (int16_t)val_l;
        dest[i] = sample_l; 
        
        int32_t abs_l = std::abs((int32_t)sample_l);
        sum_abs_left += abs_l;
        if (abs_l > peak_left) peak_left = abs_l;

        // 右声道提取
        int32_t val_r = bit32_buffer[i * 2 + 1] >> 12;
        int16_t sample_r = (val_r > INT16_MAX) ? INT16_MAX : (val_r < -INT16_MAX) ? -INT16_MAX : (int16_t)val_r;
        int32_t abs_r = std::abs((int32_t)sample_r);
        sum_abs_right += abs_r;
        if (abs_r > peak_right) peak_right = abs_r;
    }

    // 每秒打印一次双通道电量日志
    static int log_count = 0;
    static int64_t total_l = 0, total_r = 0;
    static int total_samples = 0;
    static int32_t max_p_l = 0, max_p_r = 0;
    
    total_l += sum_abs_left;
    total_r += sum_abs_right;
    total_samples += actual_samples;
    if (peak_left > max_p_l) max_p_l = peak_left;
    if (peak_right > max_p_r) max_p_r = peak_right;

    if (++log_count >= 50) { 
        if (total_samples > 0) {
            ESP_LOGI(TAG, "L-Mag: %ld, L-Peak: %ld | R-Mag: %ld, R-Peak: %ld", 
                (long)(total_l / total_samples), (long)max_p_l,
                (long)(total_r / total_samples), (long)max_p_r);
        }
        log_count = 0;
        total_l = 0; total_r = 0; total_samples = 0; max_p_l = 0; max_p_r = 0;
    }
