...
void EmojiController::ExecuteSadAnimation() {
    ...
    if (display_) {
        try {
            ...
            // 如果需要，让舵机稍微向下低头，表示悲伤
            if (servo_controller_ != nullptr) {
                servo_controller_->HeadDown(20);
                servo_controller_->HeadUp(20);
            }
...
void EmojiController::ExecuteSillyAnimation() {
    ...
    if (display_) {
        try {
            ...
            // 如果有舵机控制器，则控制舵机歪头
            if (servo_controller_ != nullptr) {
                servo_controller_->HeadRight(10); // 右倾斜
            }
...
void EmojiController::ExecuteDeliciousAnimation() {
    ...
    if (display_) {
        try {
            ...
            if (servo_controller_ != nullptr) {
                // 左右小幅度摇摆
                servo_controller_->HeadRight(5);
                vTaskDelay(pdMS_TO_TICKS(300));
                servo_controller_->HeadLeft(5);
                vTaskDelay(pdMS_TO_TICKS(300));
                servo_controller_->HeadRight(5);
            }
...
void EmojiController::ExecuteKissyAnimation() {
    ...
    if (display_) {
        try {
            ...
            // 如果有舵机控制器，则控制舵机微微前倾
            if (servo_controller_ != nullptr) {
                servo_controller_->HeadUp(10); // 头部向前倾斜
            }
...
void EmojiController::ExecuteThinkingAnimation() {
    ...
    if (display_) {
        try {
            ...
            // 如果有舵机控制器，则控制舵机歪头
            if (servo_controller_ != nullptr) {
                servo_controller_->HeadMove(-10, 5); // 左倾斜，略微抬头
            }
...
void EmojiController::ExecuteFunnyAnimation() {
    ...
    if (display_) {
        try {
            ...
            // 如果有舵机控制器，则控制舵机歪头
            if (servo_controller_ != nullptr) {
                servo_controller_->HeadMove(10, 5); // 右倾斜，略微抬头
            }
