...
// 舵机引脚配置 (N16R8 板型 11-14 和 17-18 均被 PSRAM 占用，必须改用空闲引脚)
#define SERVO_HORIZONTAL_PIN GPIO_NUM_1   // 水平舵机还原到 GPIO 1
#define SERVO_VERTICAL_PIN GPIO_NUM_2     // 垂直舵机还原到 GPIO 2
...
#define SERVO_MIN_Y (SERVO_CENTER_Y - 30)  // 增加垂直移动幅度 (原来是 -20)
#define SERVO_MAX_Y (SERVO_CENTER_Y + 30)  // 增加垂直移动幅度 (原来是 +20)
...
// 舵机方向反转配置
#define SERVO_X_INVERSE 1                // 水平轴反转：如果日志向左实际向右，设为 1
#define SERVO_Y_INVERSE 0                // 垂直轴反转：如果日志向上实际向下，设为 1

// 调试开关：若设为 1，则跳过显示屏、WiFi 和 AI 协议初始化，仅保留舵机
