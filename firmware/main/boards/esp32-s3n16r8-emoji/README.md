...
本开发板集成了PAJ7620U2手势识别传感器，与SSD1306显示屏共用I2C总线，实现手势控制舵机功能。引脚配置定义在[`board_config.h`](board_config.h)中。

| PAJ7620U2引脚 | ESP32 S3 N16R8引脚 | 配置定义 |
|------------|---------------------|----------|
| VCC        | 3.3V                | 硬件连接 |
| GND        | GND                 | 硬件连接 |
| SCL        | GPIO42              | `GESTURE_I2C_SCL_PIN` |
| SDA        | GPIO41              | `GESTURE_I2C_SDA_PIN` |
| INT        | 悬空              | 未使用 |

**注意**：PAJ7620U2传感器与SSD1306显示屏共用相同的I2C引脚，系统已处理I2C总线共享问题。所有相关配置均在`board_config.h`中统一管理。
...
小智AI集成了PAJ7620U2手势识别传感器，支持通过手势控制舵机动作，实现更自然的人机交互体验：
...
#### 连续手势识别
- **连续左右摆动**（至少3次交替）：识别为摇头动作，触发HeadShake效果
- **连续上下摆动**（至少3次交替）：识别为点头动作，触发HeadNod效果

#### 手势操作指南
1. **基本手势**：将手掌放在传感器前方5-15cm处，做出相应手势动作
2. **连续手势**：
   - **摇头**：连续做左-右-左或右-左-右手势（至少3次交替）
   - **点头**：连续做上-下-上或下-上-下手势（至少3次交替）

#### 技术实现
- **I2C总线共享**：与SSD1306显示屏安全共用I2C总线
- **实时检测**：50ms检测间隔，响应迅速
- **智能防抖**：防止手势误触发和重复执行
- **超时机制**：2秒超时自动重置手势序列状态
- **错误处理**：完善的初始化失败处理机制
...
#### 注意事项
- 手势距离：建议5-15cm，距离过近或过远均会影响识别精度
- 手势速度：动作速度适中，过快或过慢均可能影响识别
- 光线条件：避免强光直射传感器，影响识别效果
- 连续手势：确保手势动作有明显的方向变化，每个手势之间要有短暂停顿
...
#### 新增功能
- **完整的手势识别系统**：集成PAJ7620U2传感器，实现手势控制舵机动作
- **I2C总线共享支持**：与SSD1306显示屏安全共用I2C总线，无需额外引脚
- **智能连续手势识别**：支持识别摇头、点头等复合手势动作
- **实时响应**：50ms检测间隔，反应迅速且准确

#### 手势动作映射
- **单个手势**：左/右/上/下/前 → 舵机对应方向运动
- **连续手势**：左右交替 → 摇头；上下交替 → 点头
- **智能超时**：2秒超时自动重置状态，避免卡死

#### 技术亮点
- **独立任务处理**：手势识别在独立任务中运行，不影响主程序
- **错误容错**：完善的初始化失败处理和错误恢复机制
- **内存优化**：低内存占用，高效的状态机设计
- **日志系统**：详细的调试日志，方便问题排查和性能监控
