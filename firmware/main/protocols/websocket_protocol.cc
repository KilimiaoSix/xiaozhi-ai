...
WebsocketProtocol::WebsocketProtocol() {
    ...
    esp_timer_create_args_t keepalive_timer_args = {
        .callback = [](void* arg) {
            ...
            Application::GetInstance().Schedule([protocol]() {
    ...
    esp_timer_create(&keepalive_timer_args, &keepalive_timer_);
}
...
bool WebsocketProtocol::Start() {
    // 常连模式：开机即连并持续保活，服务端才能在设备空闲时主动下发消息
    esp_timer_start_periodic(keepalive_timer_, (uint64_t)WEBSOCKET_KEEPALIVE_TICK_MS * 1000);

    // 首连失败不阻断启动流程，由保活定时器重试
    if (!Connect(false)) {
...
void WebsocketProtocol::OnKeepaliveTick() {
    if (IsAudioChannelOpened()) {
        reconnect_ticks_ = 0;
        if (++ping_ticks_ >= WEBSOCKET_PING_INTERVAL_TICKS) {
            ping_ticks_ = 0;
            // 应用层 ping：服务端据此刷新 last_activity_time，回的 pong 又刷新本端 IsTimeout
            SendText("{\"type\":\"ping\"}");
        }
        return;
    }
    ...
    ping_ticks_ = 0;
    // 只在空闲态重连，避免打断正在进行的会话或固件升级
    if (Application::GetInstance().GetDeviceState() != kDeviceStateIdle) {
        reconnect_ticks_ = 0;
        return;
    }
    if (++reconnect_ticks_ < WEBSOCKET_RECONNECT_INTERVAL_TICKS) {
        return;
    }
    reconnect_ticks_ = 0;
