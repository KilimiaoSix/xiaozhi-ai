...
void Application::ToggleChatState() {
    ESP_LOGI(TAG, "ToggleChatState, current state: %s", STATE_STRINGS[device_state_]);
...
#else
    ESP_LOGI(TAG, "SERVO_ONLY_DEBUG 模式：跳过 WiFi 和网络协议初始化");
    SystemInfo::PrintHeapStats();
    SetDeviceState(kDeviceStateIdle);
#endif
...
void Application::MainEventLoop() {
    while (true) {
        ...
        if (bits & MAIN_EVENT_SEND_AUDIO) {
            ...
            if (sent_packets >= 50) {
                ESP_LOGI(TAG, "Sent 50 packets to server");
                sent_packets = 0;
            }
        }
...
void Application::SetDeviceState(DeviceState state) {
    ...
    switch (state) {
        ...
        case kDeviceStateListening:
            ...
            ESP_LOGI(TAG, "Entering Listening state, processor running: %s", 
                audio_service_.IsAudioProcessorRunning() ? "yes" : "no");
            
            // Always send start listening when explicitly entering this state from logic
